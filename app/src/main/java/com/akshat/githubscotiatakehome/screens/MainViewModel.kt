...
@HiltViewModel
class MainViewModel @Inject constructor(private val repository: GitHubScotiaRepository) :
    ViewModel() {
    val userData: MutableState<DataOrException<UsersData, Boolean, Exception>> =
        mutableStateOf(DataOrException(null, true, Exception("")))

    val userReposData: MutableState<DataOrException<List<UserReposDataItem>, Boolean, Exception>> =
        mutableStateOf(DataOrException(null, true, Exception("")))
